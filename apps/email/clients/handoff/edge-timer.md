...
export const edgeMail = new EdgeMail({
  appId: process.env.EDGE_MAIL_APP_ID ?? "edge-timer",
  endpoint: process.env.EDGE_MAIL_URL ?? "https://email.edgecoms.app",
...
| Variable | Value |
| --- | --- |
| `EDGE_MAIL_APP_ID` | `edge-timer` |
| `EDGE_MAIL_URL` | `https://email.edgecoms.app` |
| `EDGE_MAIL_SECRET` | The value of `EDGE_MAIL_SECRET_EDGE_TIMER` in the Edge Mail Vercel project. Ask the Edge team for it. **Never commit it.** |
...
| Event | When to send it for Edge Timer | `eventId` |
| --- | --- | --- |
...
| `setup.completed` | The merchant saves their first timer. Once per install. | `setup_completed:<shop>:<install id>` |
| `app.activated` | The first timer is published and showing on the storefront, the **first** time in this install. Not again if the merchant switches it off and on. | `activated:<shop>:<install id>` |
| `milestone.first_value` | The first order placed for a product while a timer was live on it. If the app cannot tell that, use the first day a published timer has been seen by 100 shoppers, and say which one you chose. **Once per shop, ever.** | `first_value:<shop>` |
...
- `app.activated` → **"Edge Timer is live on your store"**.
...
1. Install on a dev store: the log shows `{ ok: true, status: "recorded" }` for `app.installed`, and the team's inbox gets the Edge Timer welcome.
2. Complete the setup step: `setup.completed` is recorded, and no setup reminder arrives 24 hours later.
3. Make it live on the storefront: the "Edge Timer is live" email arrives.
