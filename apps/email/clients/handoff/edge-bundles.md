...
export const edgeMail = new EdgeMail({
  appId: process.env.EDGE_MAIL_APP_ID ?? "edge-bundles",
  endpoint: process.env.EDGE_MAIL_URL ?? "https://email.edgecoms.app",
...
| Variable | Value |
| --- | --- |
| `EDGE_MAIL_APP_ID` | `edge-bundles` |
| `EDGE_MAIL_URL` | `https://email.edgecoms.app` |
| `EDGE_MAIL_SECRET` | The value of `EDGE_MAIL_SECRET_EDGE_BUNDLES` in the Edge Mail Vercel project. Ask the Edge team for it. **Never commit it.** |
...
| Event | When to send it for Edge Bundles | `eventId` |
| --- | --- | --- |
...
| `setup.completed` | The merchant saves their first bundle or volume-tier offer (a draft counts). Once per install. | `setup_completed:<shop>:<install id>` |
| `app.activated` | The first bundle is published and showing on the storefront, the **first** time in this install. Not again if the merchant switches it off and on. | `activated:<shop>:<install id>` |
| `milestone.first_value` | The first order that contains a bundle or volume-tier offer from this app. **Once per shop, ever.** | `first_value:<shop>` |
...
- `app.activated` → **"Edge Bundles is live on your store"**.
...
1. Install on a dev store: the log shows `{ ok: true, status: "recorded" }` for `app.installed`, and the team's inbox gets the Edge Bundles welcome.
2. Complete the setup step: `setup.completed` is recorded, and no setup reminder arrives 24 hours later.
3. Make it live on the storefront: the "Edge Bundles is live" email arrives.
