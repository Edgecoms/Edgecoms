...
function EdgeCurrencyHeroSection() {
	...
	const CURRENCIES = [
		{ code: "USD", flag: "🇺🇸", symbol: "$", raw: "48.27", rounded: "49.00", label: "United States" },
		{ code: "EUR", flag: "🇪🇺", symbol: "€", raw: "44.62", rounded: "45.00", label: "Eurozone" },
		{ code: "GBP", flag: "🇬🇧", symbol: "£", raw: "38.15", rounded: "39.00", label: "United Kingdom" },
		{ code: "JPY", flag: "🇯🇵", symbol: "¥", raw: "7214", rounded: "7200", label: "Japan" },
		{ code: "CAD", flag: "🇨🇦", symbol: "$", raw: "64.88", rounded: "65.00", label: "Canada" },
	];

	const selected = CURRENCIES.find((c) => c.code === activeCurrency) ?? CURRENCIES[0];
			...
			<Frame className="relative pt-16 pb-12 sm:pt-20 sm:pb-16">
				...
				<div className="mx-auto max-w-[1080px] px-4 sm:px-6">
					<div className="flex flex-col items-start text-left max-w-[540px] z-20 relative">
						<span className="inline-flex items-center gap-1.5 rounded-full border border-cyan-200/80 bg-cyan-50 px-3 py-1 font-semibold text-xs text-cyan-700 shadow-2xs">
						...
						<div className="mt-6 flex items-center gap-3">
							<a
								href={product?.appStoreUrl ?? "https://apps.shopify.com/edge-currency"}
								target="_blank"
					...
					<div className="relative mt-10 sm:mt-6 w-full max-w-xl mx-auto rounded-3xl border border-neutral-200/90 bg-white shadow-xl p-6 sm:p-8">
						<div className="flex items-center justify-between border-neutral-200 border-b pb-4">
							...
							<span className="rounded-full bg-cyan-50 border border-cyan-200 px-3 py-1 font-mono font-bold text-xs text-cyan-700">
						...
						<div className="mt-6 rounded-2xl border border-neutral-200 bg-neutral-50/50 p-5 grid grid-cols-2 gap-4">
							<div className="flex flex-col gap-1 border-neutral-200 border-r pr-4">
								<span className="text-[10px] text-neutral-400 font-mono uppercase tracking-wider">Unrounded FX Math</span>
								<span className="font-mono text-xl text-neutral-400 line-through">
									{selected.symbol}{selected.raw}
								</span>
								<span className="text-[10px] text-rose-500 font-medium">Looks calculated & unpolished</span>
							</div>

							<div className="flex flex-col gap-1 pl-2">
								<span className="text-[10px] text-cyan-700 font-bold uppercase tracking-wider">Edge Smart Rounding</span>
								<span className="font-mono font-bold text-2xl text-neutral-900">
									{selected.symbol}{selected.rounded}
								</span>
								<span className="text-[10px] text-emerald-700 font-semibold">Clean localized merchant price ✓</span>
							</div>
						</div>
					</div>
				</div>
