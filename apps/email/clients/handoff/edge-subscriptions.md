...
export const edgeMail = new EdgeMail({
  appId: process.env.EDGE_MAIL_APP_ID ?? "edge-subscriptions",
  endpoint: process.env.EDGE_MAIL_URL ?? "https://email.edgecoms.app",
...
| Variable | Value |
| --- | --- |
| `EDGE_MAIL_APP_ID` | `edge-subscriptions` |
| `EDGE_MAIL_URL` | `https://email.edgecoms.app` |
| `EDGE_MAIL_SECRET` | The value of `EDGE_MAIL_SECRET_EDGE_SUBSCRIPTIONS` in the Edge Mail Vercel project. Ask the Edge team for it. **Never commit it.** |
...
| Event | When to send it for Edge Subscriptions | `eventId` |
| --- | --- | --- |
...
| `setup.completed` | The merchant creates their first subscription plan. Once per install. | `setup_completed:<shop>:<install id>` |
| `app.activated` | A subscription plan is attached to a product and the subscribe option is showing on the storefront, the **first** time in this install. Not again if the merchant switches it off and on. | `activated:<shop>:<install id>` |
| `milestone.first_value` | The first subscription contract a shopper creates (the store's first subscriber). **Once per shop, ever.** | `first_value:<shop>` |
...
- `app.activated` → **"Edge Subscriptions is live on your store"**.
...
1. Install on a dev store: the log shows `{ ok: true, status: "recorded" }` for `app.installed`, and the team's inbox gets the Edge Subscriptions welcome.
2. Complete the setup step: `setup.completed` is recorded, and no setup reminder arrives 24 hours later.
3. Make it live on the storefront: the "Edge Subscriptions is live" email arrives.
