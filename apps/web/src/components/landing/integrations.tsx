...
export function Integrations() {
	...
	return (
		<section className="relative w-full overflow-hidden border-neutral-200 border-b bg-slate-50/60">
			...
			<Frame className="relative py-20 lg:py-24">
				<div className="grid w-full grid-cols-1 items-center gap-12 px-6 sm:px-8 lg:grid-cols-2 lg:px-12">
					<div className="flex flex-col gap-5">
						...
						<p className="max-w-[420px] text-pretty text-[17px] text-neutral-600 leading-relaxed">
							Every app is a Shopify App Block, billed on the invoice you
							already receive. Trackproof sends server-side conversions on to
							the ad platforms, so they see the sales they actually drove.
						</p>
						<Link
					...
					<div className="relative z-10 -mx-6 flex w-[calc(100%+3rem)] justify-center overflow-hidden sm:mx-0 sm:w-full">
						{/* Staggered Icon Grid: Un-shrunk full-size tiles on mobile showing 3-4 blocks with edge clipping */}
						<div className="grid w-[560px] shrink-0 grid-cols-7 gap-3.5 [mask-image:radial-gradient(ellipse_80%_80%_at_50%_50%,black_60%,transparent)] sm:w-full sm:max-w-[620px]">
							{Array.from({ length: CELLS }, (_, index) => {
								const slotIndex = SLOTS.indexOf(
									index as (typeof SLOTS)[number]
								);
