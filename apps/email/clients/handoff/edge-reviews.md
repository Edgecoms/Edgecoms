...
export const edgeMail = new EdgeMail({
  appId: process.env.EDGE_MAIL_APP_ID ?? "edge-reviews",
  endpoint: process.env.EDGE_MAIL_URL ?? "https://email.edgecoms.app",
...
| Variable | Value |
| --- | --- |
| `EDGE_MAIL_APP_ID` | `edge-reviews` |
| `EDGE_MAIL_URL` | `https://email.edgecoms.app` |
| `EDGE_MAIL_SECRET` | The value of `EDGE_MAIL_SECRET_EDGE_REVIEWS` in the Edge Mail Vercel project. Ask the Edge team for it. **Never commit it.** |
...
| Event | When to send it for Edge Reviews | `eventId` |
| --- | --- | --- |
...
| `setup.completed` | The merchant turns on automatic review requests, or completes their first review import. Once per install. | `setup_completed:<shop>:<install id>` |
| `app.activated` | A review widget is live on the storefront (product page, collection cards or cart), the **first** time in this install. Not again if the merchant switches it off and on. | `activated:<shop>:<install id>` |
| `milestone.first_value` | The first review collected from a real buyer through this app (imported reviews do not count). **Once per shop, ever.** | `first_value:<shop>` |
...
- `app.activated` → **"Edge Reviews is live on your store"**.
...
1. Install on a dev store: the log shows `{ ok: true, status: "recorded" }` for `app.installed`, and the team's inbox gets the Edge Reviews welcome.
2. Complete the setup step: `setup.completed` is recorded, and no setup reminder arrives 24 hours later.
3. Make it live on the storefront: the "Edge Reviews is live" email arrives.
