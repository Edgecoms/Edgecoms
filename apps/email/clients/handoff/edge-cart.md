...
export const edgeMail = new EdgeMail({
  appId: process.env.EDGE_MAIL_APP_ID ?? "edge-cart",
  endpoint: process.env.EDGE_MAIL_URL ?? "https://email.edgecoms.app",
...
| Variable | Value |
| --- | --- |
| `EDGE_MAIL_APP_ID` | `edge-cart` |
| `EDGE_MAIL_URL` | `https://email.edgecoms.app` |
| `EDGE_MAIL_SECRET` | The value of `EDGE_MAIL_SECRET_EDGE_CART` in the Edge Mail Vercel project. Ask the Edge team for it. **Never commit it.** |
...
| Event | When to send it for Edge Cart | `eventId` |
| --- | --- | --- |
...
| `setup.completed` | The merchant saves their first upsell, add-on or reward (free-shipping bar) in the cart. Once per install. | `setup_completed:<shop>:<install id>` |
| `app.activated` | The cart drawer is switched on and replaces the theme cart on the storefront, the **first** time in this install. Not again if the merchant switches it off and on. | `activated:<shop>:<install id>` |
| `milestone.first_value` | The first order where a shopper accepted an upsell or add-on from the cart drawer. **Once per shop, ever.** | `first_value:<shop>` |
...
- `app.activated` → **"Edge Cart is live on your store"**.
...
1. Install on a dev store: the log shows `{ ok: true, status: "recorded" }` for `app.installed`, and the team's inbox gets the Edge Cart welcome.
2. Complete the setup step: `setup.completed` is recorded, and no setup reminder arrives 24 hours later.
3. Make it live on the storefront: the "Edge Cart is live" email arrives.
